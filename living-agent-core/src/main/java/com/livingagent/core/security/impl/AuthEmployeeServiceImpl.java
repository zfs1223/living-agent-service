...
public class AuthEmployeeServiceImpl implements AuthEmployeeService, EmployeeAuthService {

    private static final Logger log = LoggerFactory.getLogger(AuthEmployeeServiceImpl.class);

    private final Map<String, SecurityIdentity> employeeStore = new ConcurrentHashMap<>();
    ...
    @Override
    public SecurityIdentity createEmployee(SecurityIdentity employee) {
    ...
    @Override
    public SecurityIdentity updateEmployee(SecurityIdentity employee) {
        String id = employee.getEmployeeId();
        if (!employeeStore.containsKey(id)) {
            throw new IllegalArgumentException("Employee not found: " + id);
        }

        SecurityIdentity oldEmployee = employeeStore.get(id);
    ...
    public void deleteEmployee(String employeeId) {
        SecurityIdentity employee = employeeStore.remove(employeeId);
        if (employee != null) {
    ...
    @Override
    public Optional<SecurityIdentity> findById(String employeeId) {
        return Optional.ofNullable(employeeStore.get(employeeId));
    }

    @Override
    public Optional<SecurityIdentity> findByPhone(String phone) {
    ...
    @Override
    public Optional<SecurityIdentity> findByVoicePrintId(String voicePrintId) {
    ...
    @Override
    public Optional<SecurityIdentity> findByOAuth(String provider, String oauthUserId) {
    ...
    @Override
    public List<SecurityIdentity> findByDepartment(String department) {
    ...
    @Override
    public List<SecurityIdentity> findByIdentity(UserIdentity identity) {
        return employeeStore.values().stream()
    ...
    @Override
    public List<SecurityIdentity> findAllActive() {
        return employeeStore.values().stream()
            .filter(SecurityIdentity::isActive)
            .collect(Collectors.toList());
    }

    @Override
    public List<SecurityIdentity> findAll() {
    ...
    @Override
    public void handleAiDetectedChange(String employeeId, AuthEmployeeService.ChangeType changeType, String detectedFrom, String details) {
        findById(employeeId).ifPresent(employee -> {
    ...
    @Override
    public List<SecurityIdentity> getEmployeesNeedingSync() {
        return employeeStore.values().stream()
    ...
    public boolean hasFounder() {
        return employeeStore.values().stream().anyMatch(SecurityIdentity::isFounder);
    }

    private void updateIndexes(SecurityIdentity employee) {
        String id = employee.getEmployeeId();
    ...
    private void cleanIndexes(SecurityIdentity employee) {
        if (employee.getPhone() != null) {
