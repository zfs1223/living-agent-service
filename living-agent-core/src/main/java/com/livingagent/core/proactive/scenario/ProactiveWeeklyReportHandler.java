...
/**
 * ProactiveWeeklyReportHandler - 主动预判周报场景处理器
 *
 * 负责定时调度和生成周报，属于主动预判(proactive)领域。
 * 与 scenario.impl.WeeklyReportScenarioHandler（场景编排处理）职责不同：
 * - 本处理器: 定时触发、通知推送
 * - ScenarioHandler: 场景接口实现、任务规划
 */
public class ProactiveWeeklyReportHandler {

    private static final Logger log = LoggerFactory.getLogger(ProactiveWeeklyReportHandler.class);

    private final ProactiveTaskScheduler taskScheduler;
    private final List<AlertNotifier> notifiers;
    private final ZoneId timezone;

    public ProactiveWeeklyReportHandler(ProactiveTaskScheduler taskScheduler, List<AlertNotifier> notifiers) {
