...
| 身份 | 访问级别 | 模型选择 |
|------|----------|----------|
| INTERNAL_ACTIVE | DEPARTMENT | 动态选择(部门模型) |
| INTERNAL_PROBATION | LIMITED | 动态选择(受限模型) |
| INTERNAL_DEPARTED | CHAT_ONLY | 动态选择(闲聊模型) |
| EXTERNAL_VISITOR | CHAT_ONLY | 动态选择(闲聊模型) |
| EXTERNAL_PARTNER | LIMITED | 动态选择(受限模型) |
...
- `documents/shared/governance/01-employee-governance.md` 与 `documents/shared/governance/06-security-governance.md` 明确：员工治理与安全权限必须统一看待，员工不是只存在于权限系统中的身份条目，而是治理主体。
- `documents/shared/company/hr-15-founder-enterprise-system.md`、`hr-16-digital-employee-strategy.md`、`hr-17-digital-employee-performance.md` 对董事长、数字员工、绩效、授权、编制边界都有更强约束，因此员工模型中应把“治理授权”视为编制与实例之外的现实约束。
- `documents/shared/company/fixed-employee-duty-card-template.md` 可作为岗位编制模板的实际参考。

## 十、建议修补点

- 明确“编制”是能力定义，“实例”是运行实体，二者都有治理约束
- 明确员工与神经元不是同义词，神经元是员工的内部执行/协作投影
- 明确 `AccessLevel` 是员工可触达资源的总边界，但岗位职责卡和治理制度可以再收紧
- 增加数字员工实例生命周期说明：创建、激活、休眠、恢复、销毁

## 十一、结论

统一员工模型已经具备完整骨架，但要与企业生命体真正一致，必须把员工编制、实例、职责卡、治理授权、部门协作和通道订阅全部纳入同一套组织治理框架，而不是仅仅把它当作一组实体类。
