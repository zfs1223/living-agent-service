...
| 编号 | 问题 | 修复文件 | 状态 |
|------|------|----------|------|
| FIX-1 | `onReceiptRecorded` 监听路径 `executionResult=null` | `DepartmentChatService.java` | ✅ 已部署 |
| FIX-2 | 前端不处理 `execution_event` | `DepartmentChatInline.tsx` + `DepartmentDetail.tsx` | ✅ 已部署 |
| FIX-3 | `SelfImprovingTool` NPE (`Map.of` 不允许 null) | `SelfImprovingTool.java` | ✅ 已部署 |
| FIX-4 | MainBrain 工具迭代限制过低 (5→20) | `MainBrain.java` | ✅ 已部署 |
| FIX-5 | `DefaultExecutionReceiptReviewer` 降级验收太宽松 | `DefaultExecutionReceiptReviewer.java` | ✅ 已部署 |
| FIX-6 | WebSocket 僵尸会话循环（前端无心跳） | `DepartmentChatInline.tsx` | ✅ 已部署 |
| FIX-7 | 员工 delegateBrain 绑定无部门匹配校验 | `FixedEmployeeRegistry.java` | ✅ 已部署 |
| FIX-8 | 模型健康摘要观测缺失 | `GatewayConfig.java` | ✅ 已部署 |
| FIX-9 | 调试日志噪声偏高 | `application.yml` | ✅ 已部署 |
| **FIX-10** | **员工状态模型混乱：ONLINE/OFFLINE 与工作状态混用导致所有员工都在工作区** | **`EmployeeStatus.java` + `status.ts` + `EmployeeNeuron.java` + `DepartmentWebSocketHandler.java`** | **✅ 已部署** |
| **FIX-11** | **Embedding 模型被 chat 探针批量探测 → 连续失败 → 冷却 5min → 知识检索/语义搜索不可用** | **`ModelPerformanceAssessorImpl.java`** | **✅ 已部署** |
...
- **验证**：[DepartmentChatService.java#L260-L261](file:///f:/SoarCloudAI/docker/living-agent-service/living-agent-gateway/src/main/java/com/livingagent/gateway/service/DepartmentChatService.java#L260-L261) 中 `triggerAsyncFinalResponse(executionResult.executionId(), sessionId)` 在所有 receipt 收齐时被调用
- **状态**：✅ 已部署
...
- **验证**：`HEARTBEAT_INTERVAL_MS = 30000` (L184) + `ws.send({ type: 'ping' })` (L213)
- **状态**：✅ 已部署
...
- **验证**：`validateDelegateBrainBindings()` 完整输出 total/bound/unbound/mismatch 四类计数 (L960-L1010)
- **状态**：✅ 已部署
...
- **验证**：`coordinate()`(L43) / `needsCrossDepartmentCoordination()`(L124) 已实现
- **状态**：✅ 已部署
...
- **验证**：`@Scheduled(fixedRate = 300000, initialDelay = 60000)` (L391) + `modelHealthRegistry` Bean (L333)
- **状态**：✅ 已部署
...
- **代码位置**：`frontend/src/pages/DepartmentDetail/DepartmentChatInline.tsx`
- **验证**：state(L64) / 处理(L292-293) / 展示(L517-533) 完整链路
- **状态**：✅ 已部署
...
- **验证**：`isRequirementFrozen()`(L110) / `withRequirementStatus()`(L125) / `withIncrementedVersion()`(L152) / `activeSessionPlans` Map(L102) / `requirement_frozen` 事件(L421) 全部就位
- **状态**：✅ 已部署
...
| 日期 | 修复项 | 状态 |
|------|--------|------|
| 2026-06-08 | FIX-1~4: receipt缓存/前端事件/NPE/迭代限制 | ✅ 已部署 |
| 2026-06-08 | FIX-5: ReceiptReviewer 降级验收严格化 | ✅ 已部署 |
| 2026-06-08 | FIX-6: WebSocket 30秒心跳 | ✅ 已部署 |
| 2026-06-08 | FIX-7: 员工绑定部门匹配校验 | ✅ 已部署 |
| 2026-06-08 | FIX-8: 模型健康5分钟周期摘要 | ✅ 已部署 |
| 2026-06-08 | FIX-9: 日志降噪 | ✅ 已部署 |
| 2026-06-08 | P1-4: CrossDepartmentCoordinator 跨部门协调 | ✅ 已部署 |
| 2026-06-08 | P2-3: 前端需求状态展示 | ✅ 已部署 |
| 2026-06-08 | P2-4: 需求冻结/防漂移逻辑 | ✅ 已部署 |
| 2026-06-09 | FIX-10: 员工状态模型双维度重构 | ✅ 已部署 |
| 2026-06-09 | FIX-11: Embedding 模型批量探测改用 /embeddings | ✅ 已部署 |

---

## 归档说明

> 本文档所有可编码修复项已全部完成（含本次 FIX-11 追加）。  
> 仅 **P1-3: DockerSandboxService** 需运维介入（Docker-in-Docker 宿主 socket 配置），保留在文中。
>
> 本文件归档到 `docs/old/ACTIVE_FIXES_TODO_2026-06-09.md`。
