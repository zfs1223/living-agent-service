...
| # | 任务 | 依赖 | 验收 |
|---|------|------|------|
| 2.1 | Dashboard 成本分析接真实数据（P1-1） | 1.1 | 成本卡片真实 |
| 2.2 | 绩效评分算法实现（P1-2） | 无 | score/achievementRate 非 0 |
| 2.3 | 绩效 Bean 去重（P1-4） | 2.2 | 仅 JPA 实现生产可用 |
| 2.4 | Embedding 生产硬失败（P2-1） | 无 | 缺模型启动失败 |
