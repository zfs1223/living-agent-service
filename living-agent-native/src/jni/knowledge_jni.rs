...
fn throw_exception(env: &mut Env<'_>, class_name: &str, message: &str) {
...
pub extern "system" fn Java_com_livingagent_core_knowledge_nativestore_NativeKnowledge_init(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<()> {
            let path = match jstring_to_string(env, &db_path) {
                Ok(s) => s,
                Err(_) => {
                    throw_exception(env, "java/lang/IllegalArgumentException", "Invalid db_path");
                    return Ok(())
                }
            };
...
pub extern "system" fn Java_com_livingagent_core_knowledge_nativestore_NativeKnowledge_store(
    ...
    key: JString<'_>,
    content: JString<'_>,
    knowledge_type: JString<'_>,
    brain_domain: JString<'_>,
    importance: JString<'_>,
    validity: JString<'_>,
    vector: JByteArray<'_>,
) -> bool {
    unowned_env
        .with_env(|env| -> jni::errors::Result<bool> {
            ...
            let type_str = match jstring_to_string(env, &knowledge_type) {
                Ok(s) => s,
            ...
            if let Ok(domain) = jstring_to_string(env, &brain_domain) {
                if !domain.is_empty() {
                    entry = entry.with_brain_domain(domain);
                }
            }

            if let Ok(imp) = jstring_to_string(env, &importance) {
            ...
            if let Ok(val) = jstring_to_string(env, &validity) {
            ...
            let vector_data: Vec<f32> = if !vector.is_null() {
                let bytes = match jbyte_array_to_bytes(env, &vector) {
                    Ok(b) => b,
                    Err(_) => Vec::new(),
                };
                bytes
...

            {
                ...
                if let Some(cache) = cache_guard.as_ref() {
                    if let Some(entry) = cache.get(&key_str) {
                        return Ok(entry_to_jobject(env, &entry));
                    }
                }
            }
        ...
        .with_env(|env| -> jni::errors::Result<JObject<'_>> {
            ...
            if let Some(backend) = backend_guard.as_ref() {
                match backend.search(&query_str, limit as usize) {
                    Ok(entries) => {
                        let list = create_java_list(env);

                        for entry in entries {
                            let jentry = entry_to_jobject(env, &entry);
...
pub extern "system" fn Java_com_livingagent_core_knowledge_nativestore_NativeKnowledge_vectorSearch(
    mut unowned_env: EnvUnowned<'_>,
    _class: JClass<'_>,
    query_vector: JByteArray<'_>,
    top_k: i32,
    threshold: f32,
) -> jni::sys::jobject {
    unowned_env
        .with_env(|env| -> jni::errors::Result<JObject<'_>> {
            let bytes = match jbyte_array_to_bytes(env, &query_vector) {
                Ok(b) => b,
                Err(_) => return Ok(JObject::null()),
            };

            let query: Vec<f32> = bytes
                .chunks(4)
                .filter_map(|chunk| {
                    if chunk.len() == 4 {
                        Some(f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
            ...
            if let Some(vs) = vs_guard.as_ref() {
                ...
                for result in results {
                    let jentry = entry_to_jobject(env, &result.entry);
                    let _ = env.call_method(
                        &list,
                        &JNIString::from("add"),
                        jni_sig!("(Ljava/lang/Object;)Z"),
...
pub extern "system" fn Java_com_livingagent_core_knowledge_nativestore_NativeKnowledge_cosineSimilarity(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<f32> {
            let bytes1 = match jbyte_array_to_bytes(env, &vec1) {
                Ok(b) => b,
                Err(_) => return Ok(0.0),
            };

            let bytes2 = match jbyte_array_to_bytes(env, &vec2) {
                Ok(b) => b,
                Err(_) => return Ok(0.0),
            };

            let v1: Vec<f32> = bytes1
                .chunks(4)
                .filter_map(|chunk| {
                    if chunk.len() == 4 {
                        Some(f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<JObject<'_>> {
            ...
            if let Some(cache) = cache_guard.as_ref() {
                ...
                let _ = put_map_long(env, &map, "hits", stats.hits as i64);
                let _ = put_map_long(env, &map, "misses", stats.misses as i64);
                let _ = put_map_double(env, &map, "hit_rate", stats.hit_rate as f64);
...
// ===========================================
// 辅助函数（在 with_env 闭包内部使用）
// ===========================================

fn create_java_list<'local>(env: &mut Env<'local>) -> JObject<'local> {
    let class = env.find_class(&JNIString::from("java/util/ArrayList")).unwrap();
    env.new_object(&class, jni_sig!("()V"), &[]).unwrap()
}

fn create_java_map<'local>(env: &mut Env<'local>) -> JObject<'local> {
    let class = env.find_class(&JNIString::from("java/util/HashMap")).unwrap();
    env.new_object(&class, jni_sig!("()V"), &[]).unwrap()
}

fn put_map_long(env: &mut Env<'_>, map: &JObject<'_>, key: &str, value: i64) -> Result<(), String> {
...
fn put_map_double(env: &mut Env<'_>, map: &JObject<'_>, key: &str, value: f64) -> Result<(), String> {
    let jkey = env.new_string(key).map_err(|e| e.to_string())?;
...
fn entry_to_jobject<'local>(env: &mut Env<'local>, entry: &KnowledgeEntry) -> JObject<'local> {
    let map = create_java_map(env);
...
fn put_map_string(env: &mut Env<'_>, map: &JObject<'_>, key: &str, value: &str) -> Result<(), String> {
    let jkey = env.new_string(key).map_err(|e| e.to_string())?;
