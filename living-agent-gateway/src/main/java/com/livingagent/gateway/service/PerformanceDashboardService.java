...
import com.livingagent.core.operation.performance.InMemoryPerformanceAssessmentService;
...
public class PerformanceDashboardService {
    ...
    public Map<String, Object> companySummary(String departmentId, int limit) {
        ...
        payload.put("topPerformers", departmentId != null ? assessmentService.getTopPerformers(departmentId, limit) : getCompanyTop(limit));
        payload.put("bottomPerformers", departmentId != null ? assessmentService.getBottomPerformers(departmentId, limit) : getCompanyBottom(limit));
    ...
    public Map<String, Object> buildSummary(String departmentId, int limit) {
        List<PerformanceAssessmentService.EmployeeRanking> top = departmentId != null ? assessmentService.getTopPerformers(departmentId, limit) : getCompanyTop(limit);
        List<PerformanceAssessmentService.EmployeeRanking> bottom = departmentId != null ? assessmentService.getBottomPerformers(departmentId, limit) : getCompanyBottom(limit);
    ...
    private List<PerformanceAssessmentService.EmployeeRanking> getCompanyTop(int limit) {
        if (assessmentService instanceof InMemoryPerformanceAssessmentService memory) {
            return (List<PerformanceAssessmentService.EmployeeRanking>) (List<?>) memory.getCompanyTopPerformers(limit);
        }
        return List.of();
    }

    @SuppressWarnings("unchecked")
    private List<PerformanceAssessmentService.EmployeeRanking> getCompanyBottom(int limit) {
        if (assessmentService instanceof InMemoryPerformanceAssessmentService memory) {
            return (List<PerformanceAssessmentService.EmployeeRanking>) (List<?>) memory.getCompanyBottomPerformers(limit);
        }
        return List.of();
    }
}
