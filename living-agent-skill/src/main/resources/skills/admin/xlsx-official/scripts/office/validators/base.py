...
class BaseSchemaValidator:
    ...
    def validate_unique_ids(self):
        ...
        for xml_file in self.xml_files:
            try:
                file_errors, global_ids = self._validate_file_unique_ids(
                    xml_file, global_ids
                )
    ...
    def _validate_file_unique_ids(self, xml_file, global_ids):
        ...
        for elem in root.iter():
            ...
            if self._is_in_excluded_container(elem):
                continue
            ...
            if scope == "global":
                errors, global_ids = self._validate_global_id(
                    xml_file, elem, tag, id_value, global_ids, errors
                )
            elif scope == "file":
                errors, file_ids = self._validate_file_id(
                    xml_file, elem, tag, attr_name, id_value, file_ids, errors
                )

        return errors, global_ids
    ...
    def _validate_global_id(self, xml_file, elem, tag, id_value, global_ids, errors):
    ...
    def _validate_file_id(self, xml_file, elem, tag, attr_name, id_value, file_ids, errors):
        key = (tag, attr_name)
    ...
    def validate_file_references(self):
        ...
        all_files = self._get_target_files()
        ...
        for rels_file in rels_files:
            try:
                referenced_files, broken_refs = self._parse_rels_file(rels_file)
                all_referenced_files.update(referenced_files)
                errors.extend(self._format_broken_refs(rels_file, broken_refs))
            except Exception as e:
                rel_path = rels_file.relative_to(self.unpacked_dir)
                errors.append(f"  Error parsing {rel_path}: {e}")

        errors.extend(self._find_unreferenced_files(all_files, all_referenced_files))
    ...
    def _parse_rels_file(self, rels_file):
        ...
        for rel in rels_root.findall(
            ...
            if target and not target.startswith(("http", "mailto:")):
                target_path = self._resolve_target_path(target, rels_file, rels_dir)
    ...
    def _resolve_target_path(self, target, rels_file, rels_dir):
        if target.startswith("/"):
            return self.unpacked_dir / target.lstrip("/")
        elif rels_file.name == ".rels":
            return self.unpacked_dir / target
        else:
    ...
    def validate_content_types(self):
        ...
        try:
            ...
            declared_parts, declared_extensions = self._parse_content_types_declarations(root)
            errors.extend(self._validate_xml_root_elements(declared_parts))
            errors.extend(self._validate_file_extensions(declared_extensions))
        except Exception as e:
    ...
    def _parse_content_types_declarations(self, root):
        ...
        return declared_parts, declared_extensions
